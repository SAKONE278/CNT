...
router.post('*', async (req, res) => {
  ...
  if (isEmpty(email)) {
    return res
      .status(404)
      .json({ status: 'error', email: getTranslation({ dictionary, code: 'email-required', language }) });
  }

  if (!validator.isEmail(email)) {
    return res
      .status(400)
      .json({ status: 'error', email: getTranslation({ dictionary, code: 'invalid-email', language }) });
  }

  if (!code) {
    return res
      .status(404)
      .json({ status: 'error', code: getTranslation({ dictionary, code: 'code-required', language }) });
  }
  ...
  if (!user) {
    return res
      ...
      .json({
        status: 'error',
        code: getTranslation({ dictionary, code: 'the-associated-user-is-no-longer-valid', language }),
      });
  }
  ...
  if (!authCode) {
    return res
      .status(404)
      .json({ status: 'error', code: getTranslation({ dictionary, code: 'code-not-found', language }) });
  }

  if (moment(authCode.expires).isBefore(moment())) {
    return res
      .status(400)
      .json({ status: 'error', code: getTranslation({ dictionary, code: 'code-expired', language }) });
  }

  if (password.length < 6) {
    return res
      .status(400)
      .json({ status: 'error', password: getTranslation({ dictionary, code: 'password-too-short', language }) });
  }
