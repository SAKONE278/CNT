...
module.exports = async (req, res) => {
  ...
  if (file.type.includes('image') || file.type.includes('video')) {
    if (thumbConfigs) {
      for (let thumbConfig of thumbConfigs) {
        ...
        try {
          await bufferToStream(
            await sharp(preThumb || file.path)
              .resize({
                width: ['crop', 'width'].includes(thumbConfig.type) ? thumbConfig.size : width,
                height: ['crop', 'height'].includes(thumbConfig.type) ? thumbConfig.size : height,
                fit: 'cover',
              })
              .jpeg()
              .toFile(name),
  ...
  res.status(200).json({ status: 'ok', shield: result, id: result.shield });
};
